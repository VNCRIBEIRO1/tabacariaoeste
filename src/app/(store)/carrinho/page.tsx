  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span className="text-stone-900 font-medium">Carrinho</span>
      </nav>

      <h1 className="text-2xl font-bold text-stone-900 mb-8">Meu Carrinho</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-white rounded-lg border p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/produto/${item.slug}`} className="font-medium text-stone-800 hover:text-amber-700 line-clamp-1">
                  {item.name}
                </Link>
                {item.variantInfo && <p className="text-xs text-gray-500 mt-0.5">{item.variantInfo}</p>}
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center border rounded">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 hover:bg-gray-100" disabled={item.quantity <= 1}>
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="text-sm w-10 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 hover:bg-gray-100" disabled={item.quantity >= item.maxStock}>
        ...
        <div className="bg-white rounded-lg border p-6 h-fit sticky top-24">
          <h2 className="text-lg font-semibold text-stone-900 mb-4">Resumo do Pedido</h2>
